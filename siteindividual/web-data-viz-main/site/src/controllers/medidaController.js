...
function buscarUltimasinfluencer(req, res) {
    ...
    medidaModel.buscarUltimasinfluencer(influencer, limite_linhas).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
...
function buscarinfluencerEmTempoReal(req, res) {
    ...
    medidaModel.buscarinfluencerEmTempoReal(influencer).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
